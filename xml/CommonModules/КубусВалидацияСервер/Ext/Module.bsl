...
Функция ВалидацияПараметровHTTPЗапроса(Знач HTTPСервисЗапрос, ШаблонВалидацииJSON) Экспорт
	...
	Для каждого КЗ Из ШаблонВалидации Цикл
		...
		ЗначениеПараметра = HTTPСервисЗапрос.ПараметрыURL.Получить(ИмяПараметра);
...
Функция ВалидацияЗначения(Значение, ИмяПараметра, ПравилаВалидации)
	...
	// ИмяПравила:
	//  _required	true или false
	//  _type		'string', 'number', 'boolean', 'array', 'object'
	//  _minLength	Для строк и массивов
	//  _maxLength	Для строк и массивов
	//  _min		Для чисел
	//  _max		Для чисел
	//  _pattern	RegExp для строк
	//  _format		'email', 'uuid', 'ip', 'date', 'url' и т.д.
	//  _enum		Список допустимых значений
	...
	Для каждого КЗ Из ПравилаВалидации Цикл
		...
		Если МассивДопустимых.Найти(НРег(ИмяПравила)) Тогда
			Структура = Вычислить(НРег(ИмяПравила) + "(ИмяПараметра, Значение, Правило)");
			Если НЕ Структура.ВалидацияПройдена Тогда
				СтруктураВозврата.ВалидацияПройдена = Ложь;
				КубусОбщегоНазначенияКлиентСервер.ДополнитьМассив(СтруктураВозврата.ОшибкиВалидации, Структура.ОшибкиВалидации);
			КонецЕсли;
